...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var EMPTY_CONFIG = { enabled: true, cooldownMs: 300000, cooldownMaxMs: 1800000, cooldownBackoff: 2, retryOnThrottle: true, maxRetriesPerCandidate: 2, retryBackoffMs: 1000, maxSwitchesPerStep: 3, sessionLoadWeight: 1, contentAwareTier: true, contentAwareTierTextThreshold: 32768, routes: {} }
    ...
    var DICT_ZH = {
      ...
      'caps.title': '自定义供应商模型能力',
      'caps.empty': '暂无自定义供应商',
      'caps.empty.title': '未配置自定义供应商（hand-declared）。内置目录供应商的模型能力由宿主模型目录管理。',
      'caps.models': '{n} 个模型',
      'caps.unset': '未设置…',
      'caps.addLevel': '+ 添加档位',
      ...
      'caps.err.builtin': '。内置供应商的能力由宿主模型目录管理，请在宿主 Models 页面调整。',
      'caps.err.noLevel': '。至少勾选一个 off 之外的思考级别（如 low/medium/high）。',
      ...
      'routes.summary': '{n} 个 · 累计 {req} 次请求 / {sw} 次切换',
      'routes.batchToggle': '按模型名分组',
      'routes.batchTitle': '一键按模型名分组添加候选链',
      'routes.batchTip': '扫描所有供应商目录，按模型名分组。点击模型名即可一键创建路由——将该模型的所有供应商作为 tier2 候选链（自动故障转移）。多供应商的模型排在前面。',
      'routes.batchAddAll': '全部添加多供应商模型',
      'routes.batchEmpty': '目录为空或无可用模型',
      'routes.batchExists': '已添加',
      'routes.batchMulti': '{n} 家供应商',
      'routes.batchSingle': '1 家供应商',
      ...
      'task.title': '任务路由',
      'task.on': '启用任务路由（任务属性×模型属性自动路由）',
      'task.enableTip': '按任务画像（重要性/紧急性/幂等/复杂度/token 预算）匹配模型属性（能力/速度/延迟/位置/信任/价格）自动排序与过滤。默认关闭；开启前先为候选标注模型属性。',
      'task.autoComplexity': '自动复杂度（按请求体量推断）',
      'task.capability': '能力',
      'task.speed': '速度',
      'task.latency': '延迟',
      'task.location': '位置',
      ...
      'task.level.medium': '中',
      'task.level.high': '高',
      'task.level.unknown': '未知',
      'task.loc.internal': '内',
      'task.loc.external': '外',
      'task.trust.trusted': '可信',
      'task.trust.untrusted': '不可信',
      'task.wCapability': '能力权重',
      'task.wLatency': '延迟权重',
      'task.wSpeed': '速度权重',
      'task.wPrice': '价格权重',
      'task.wImportance': '重要性权重',
      'task.thresholdLow': '低复杂度阈值',
      'task.thresholdHigh': '高复杂度阈值',
      'task.pricePh': '价格',
    }
    var DICT_EN = {
      ...
      'caps.title': 'Custom provider model capabilities',
      'caps.empty': 'No custom providers',
      'caps.empty.title': 'No hand-declared providers configured. Model capabilities of built-in catalog providers are managed by the host model catalog.',
      ...
      'caps.err.builtin': ' Built-in provider capabilities are managed by the host model catalog — adjust them on the host Models page.',
      'caps.err.noLevel': ' Check at least one reasoning effort other than off (e.g. low/medium/high).',
      ...
      'routes.batchToggle': 'Group by model name',
      'routes.batchTitle': 'One-click add candidate chains by model name',
      'routes.batchTip': 'Scan all provider catalogs and group by model name. Click a model name to instantly create a route — all providers offering that model become the tier2 candidate chain (with automatic failover). Multi-provider models are listed first.',
      'routes.batchAddAll': 'Add all multi-provider models',
      'routes.batchEmpty': 'Catalog is empty or no models available',
      'routes.batchExists': 'Added',
      'routes.batchMulti': '{n} providers',
      'routes.batchSingle': '1 provider',
      'routes.summary': '{n} · {req} requests / {sw} switches',
      ...
      'task.title': 'Task routing',
      'task.on': 'Enable task routing (task attributes × model attributes)',
      'task.enableTip': 'Match task profile (importance / urgency / idempotency / complexity / token budget) against model attributes (capability / speed / latency / location / trust / price) to rank and filter candidates. Off by default; annotate candidate attributes first.',
      'task.autoComplexity': 'Auto complexity (by request size)',
      'task.capability': 'Cap.',
      'task.speed': 'Speed',
      ...
      'task.price': 'Price',
      'task.level.low': 'low',
      'task.level.medium': 'med',
      'task.level.high': 'high',
      'task.level.unknown': '?',
      'task.loc.internal': 'in',
      'task.loc.external': 'ex',
      'task.trust.trusted': 'ok',
      'task.trust.untrusted': '!',
      'task.wCapability': 'Capability weight',
      'task.wLatency': 'Latency weight',
      'task.wSpeed': 'Speed weight',
      'task.wPrice': 'Price weight',
      'task.wImportance': 'Importance weight',
      'task.thresholdLow': 'Low-complexity threshold',
      'task.thresholdHigh': 'High-complexity threshold',
      'task.pricePh': 'Price',
    }
    ...
    function Panel() {
      ...
      // 一键按模型名分组添加面板折叠状态
      var s19 = React.useState(false); var groupOpen = s19[0]; var setGroupOpen = s19[1]
      ...
      var load = React.useCallback(function () {
        ...
        api('/api/model-router/state').then(function (res) {
          ...
          setCfg({ enabled: res.config.enabled, cooldownMs: res.config.cooldownMs,
                   ...
                   sessionLoadWeight: res.config.sessionLoadWeight ?? 1,
                   contentAwareTier: res.config.contentAwareTier !== false,
                   contentAwareTierTextThreshold: res.config.contentAwareTierTextThreshold ?? 32768,
                   taskRouting: res.config.taskRouting ? JSON.parse(JSON.stringify(res.config.taskRouting)) : { enabled: false },
      ...
      var canonicalCfg = function (c) {
        ...
        Object.keys(c.routes || {}).sort().forEach(function (id) {
          ...
          ;['tier1', 'tier2', 'tier3'].forEach(function (slot) {
            norm[slot] = (r[slot] || []).map(function (cd) {
              ...
              o.location = cd.location === 'internal' ? 'internal' : 'external'
              o.trust = cd.trust === 'untrusted' ? 'untrusted' : 'trusted'
              o.speed = cd.speed || 'unknown'
              o.latency = cd.latency || 'unknown'
              o.capability = cd.capability || 'medium'
              o.price = (typeof cd.price === 'number' && cd.price > 0) ? cd.price : 0
              return o
            })
          })
        ...
        return JSON.stringify({
          ...
          sessionLoadWeight: Number(c.sessionLoadWeight) || 0,
          contentAwareTier: c.contentAwareTier !== false,
          contentAwareTierTextThreshold: Number(c.contentAwareTierTextThreshold) || 0,
          taskRouting: (function (tr) {
            tr = tr || {}
            return {
              enabled: !!tr.enabled,
              defaults: tr.defaults && typeof tr.defaults === 'object' ? tr.defaults : {},
              autoComplexity: tr.autoComplexity !== false,
              complexityThresholds: tr.complexityThresholds && typeof tr.complexityThresholds === 'object' ? tr.complexityThresholds : {},
              weights: tr.weights && typeof tr.weights === 'object' ? tr.weights : {},
            }
          })(c.taskRouting),
          routes: routes,
        })
      ...
      // ------------------------------------------------------------------
      // 一键按模型名分组添加：扫描 catalog（{ provider: [modelId, ...] }），
      // 按模型名分组，多供应商模型排前面。点击即创建路由，将该模型的所有
      // 供应商作为 tier2 候选链（标准档位，自动故障转移）。
      // ------------------------------------------------------------------
      var modelGroups = function () {
        var groups = {} // modelId -> [providerId, ...]
        for (var pid in catalog) {
          var models = catalog[pid] || []
          for (var i = 0; i < models.length; i++) {
            var mid = models[i]
            if (!groups[mid]) groups[mid] = []
            if (groups[mid].indexOf(pid) < 0) groups[mid].push(pid)
          }
        }
        return Object.keys(groups).map(function (mid) {
          return { model: mid, providers: groups[mid].sort() }
        }).sort(function (a, b) {
          // 多供应商优先，其次按模型名字母序
      ...
      var addRouteByGroup = function (mid, providers) {
        if (!mid || !cfg || cfg.routes[mid]) return
        setCfg(function (prev) {
          var next = JSON.parse(JSON.stringify(prev))
          next.routes[mid] = {
            tier1: [],
            tier2: providers.map(function (p) { return { provider: p, model: mid } }),
            tier3: [],
          }
          return next
        })
      ...
      var addAllGroups = function () {
        setCfg(function (prev) {
          ...
          modelGroups().forEach(function (g) {
            // 只添加多供应商模型且尚不存在的路由
            if (g.providers.length >= 2 && !next.routes[g.model]) {
              next.routes[g.model] = {
                tier1: [],
                tier2: g.providers.map(function (p) { return { provider: p, model: g.model } }),
                tier3: [],
              }
            }
          })
          ...
          chain.length === 0
            ...
            : chain.map(function (c, i) {
                ...
                return React.createElement('div', { className: 'dsh-mr-cand', key: i },
                  ...
                  (cfg && cfg.taskRouting && cfg.taskRouting.enabled ? [
                    React.createElement('select', { className: 'dsh-mr-sel dsh-mr-sel-task', 'aria-label': t('task.capability'), title: '能力档位：对照任务复杂度', value: c.capability || 'medium',
                      onChange: function (e) { update(i, { capability: e.target.value }) } },
                      ['low', 'medium', 'high'].map(function (v) { return React.createElement('option', { key: v, value: v }, t('task.level.' + v)) })),
                    React.createElement('select', { className: 'dsh-mr-sel dsh-mr-sel-task', 'aria-label': t('task.speed'), title: '生成速度（紧急/重要任务加分）', value: c.speed || 'unknown',
                      onChange: function (e) { update(i, { speed: e.target.value }) } },
                      ['unknown', 'low', 'medium', 'high'].map(function (v) { return React.createElement('option', { key: v, value: v }, t('task.level.' + v)) })),
                    React.createElement('select', { className: 'dsh-mr-sel dsh-mr-sel-task', 'aria-label': t('task.latency'), title: '首 token 延迟（紧急任务惩罚）', value: c.latency || 'unknown',
                      onChange: function (e) { update(i, { latency: e.target.value }) } },
                      ['unknown', 'low', 'medium', 'high'].map(function (v) { return React.createElement('option', { key: v, value: v }, t('task.level.' + v)) })),
                    React.createElement('select', { className: 'dsh-mr-sel dsh-mr-sel-task', 'aria-label': t('task.location'), title: '内部/外部模型（外部需配合脱敏插件如 dsh-redact）', value: c.location || 'external',
                      onChange: function (e) { update(i, { location: e.target.value }) } },
                      ['internal', 'external'].map(function (v) { return React.createElement('option', { key: v, value: v }, t('task.loc.' + v)) })),
                    React.createElement('select', { className: 'dsh-mr-sel dsh-mr-sel-task', 'aria-label': t('task.trust'), title: '不可信候选只在只读环境工作（确保不了只读则跳过）', value: c.trust || 'trusted',
                      onChange: function (e) { update(i, { trust: e.target.value }) } },
                      ['trusted', 'untrusted'].map(function (v) { return React.createElement('option', { key: v, value: v }, t('task.trust.' + v)) })),
                    React.createElement('input', { className: 'dsh-mr-sel dsh-mr-sel-task dsh-mr-sel-price', type: 'text', inputMode: 'numeric', placeholder: t('task.pricePh'), title: '每百万 token 成本（0 = 未知/免费，不参与预算过滤与价格排序）', value: (typeof c.price === 'number' && c.price > 0) ? String(c.price) : '',
                      onChange: function (e) {
                        var v = e.target.value.trim()
                        var n = Number(v)
                        update(i, { price: (v === '' || !Number.isFinite(n) || n <= 0) ? 0 : n })
                      } }),
                  ] : null),
                  hChip,
      ...
      var sessionLoadMap = (data && data.sessionLoad) || {}
      var runningCount = (data && data.runningSessions) || 0
      ...
      return React.createElement('div', { className: 'dsh-mr' },
        ...
        React.createElement('div', { className: 'dsh-mr-card' },
          React.createElement('div', { className: 'dsh-mr-card-head' },
            ...
            React.createElement('button', { type: 'button', className: 'dsh-mr-switch' + ((cfg && cfg.enabled) ? ' is-on' : ''),
              role: 'switch', 'aria-checked': !!(cfg && cfg.enabled), 'aria-label': t('global.master'), title: t('global.master'),
              onClick: function () { setCfg(function (p) { return Object.assign({}, p, { enabled: !p.enabled }) }) } }),
            React.createElement('b', null, t('global.title')),
            React.createElement('span', { className: 'dsh-mr-hint', title: t('global.disabledTip') }, (cfg && cfg.enabled) ? t('global.on') : t('global.off'))),
          ...
          React.createElement('label', { className: 'dsh-mr-check', title: t('global.retryTip') },
          ...
          React.createElement('label', { className: 'dsh-mr-check', title: t('global.healthTip') },
            ...
            t('global.health')),
          (cfg && cfg.healthRanking) ? React.createElement('label', { title: '会话负载权重（0-10）：运行中会话数越多的候选排名越靠后，实现 least-connections 负载均衡。设 0 禁用（退化为纯健康度排序）。推荐 1-3。' }, '会话负载权重',
            React.createElement('input', { className: 'dsh-mr-num', type: 'number', min: 0, max: 10, step: 0.5,
              name: 'sessionLoadWeight', autocomplete: 'off', inputMode: 'decimal',
              value: cfg ? cfg.sessionLoadWeight : 1,
              onChange: function (e) { setCfg(function (p) { return Object.assign({}, p, { sessionLoadWeight: Math.min(10, Math.max(0, Number(e.target.value) || 0)) }) }) } })) : null,
          (cfg && cfg.healthRanking && (cfg.sessionLoadWeight || 0) > 0) ? React.createElement('div', { className: 'dsh-mr-hint', title: '各候选模型当前运行中会话数（least-connections 负载均衡统计）' },
            '运行中会话 ' + runningCount + '：' + (Object.keys(sessionLoadMap).length > 0 ? Object.keys(sessionLoadMap).sort().map(function (k) { return k + '(' + sessionLoadMap[k] + ')' }).join('、') : '无')) : null,
          React.createElement('label', { className: 'dsh-mr-check', title: t('global.contextAwareTip') },
          ...
          (cfg && cfg.contextAware !== false) ? React.createElement('div', { className: 'dsh-mr-grid' },
          ...
          React.createElement('label', { className: 'dsh-mr-check', title: '当用户未手动选档时，根据请求内容自动选档：\n- 图片 → 跨所有档找支持图片输入的候选，选运行中会话数最少的候选所在档\n- 长文本 → 跨所有档找上下文窗口足够的候选，选运行中会话数最少的候选所在档\n- 无合适候选 → 保持默认档' },
            React.createElement('input', { type: 'checkbox', checked: cfg ? cfg.contentAwareTier !== false : true,
              onChange: function (e) { setCfg(function (p) { return Object.assign({}, p, { contentAwareTier: e.target.checked }) }) } }),
            ' 内容感知选档'),
          (cfg && cfg.contentAwareTier !== false) ? React.createElement('label', { title: '长文本阈值（token）：估算 token 数超过此值才触发上下文窗口选档。设 0 则任何文本都触发。默认 32768（≈128K 字符），短消息不触发选档切换。' }, '长文本阈值',
            React.createElement('input', { className: 'dsh-mr-num', type: 'number', min: 0, step: 4096,
              name: 'contentAwareTierTextThreshold', autocomplete: 'off', inputMode: 'numeric',
              value: cfg ? cfg.contentAwareTierTextThreshold : 32768,
              onChange: function (e) { setCfg(function (p) { return Object.assign({}, p, { contentAwareTierTextThreshold: Math.max(0, Number(e.target.value) || 0) }) }) } })) : null,
          React.createElement('div', { className: 'dsh-mr-task-row', title: t('task.enableTip') },
            React.createElement('label', { className: 'dsh-mr-check', title: t('task.enableTip') },
              React.createElement('input', { type: 'checkbox', checked: !!(cfg && cfg.taskRouting && cfg.taskRouting.enabled),
                onChange: function (e) { setCfg(function (p) {
                  var tr = p.taskRouting || {}
                  return Object.assign({}, p, { taskRouting: Object.assign({}, tr, { enabled: e.target.checked }) })
                }) } }),
              t('task.on'))),
          (cfg && cfg.taskRouting && cfg.taskRouting.enabled) ? React.createElement('div', { className: 'dsh-mr-task-grid' },
            React.createElement('label', { className: 'dsh-mr-check', title: '按请求体量（token 估算）推断复杂度：> 低阈值 = medium，>= 高阈值 = high。complexity 已显式标注时不用推断。' },
              React.createElement('input', { type: 'checkbox', checked: cfg.taskRouting.autoComplexity !== false,
                onChange: function (e) { setCfg(function (p) { var tr = p.taskRouting || {}; return Object.assign({}, p, { taskRouting: Object.assign({}, tr, { autoComplexity: e.target.checked }) }) }) } }),
              t('task.autoComplexity')),
            React.createElement('label', { title: '低复杂度阈值（token）' }, t('task.thresholdLow'),
              React.createElement('input', { className: 'dsh-mr-num', type: 'number', min: 0, step: 4096, name: 'taskThLow', autocomplete: 'off', inputMode: 'numeric',
                value: (cfg.taskRouting.complexityThresholds && cfg.taskRouting.complexityThresholds.low) || 16384,
                onChange: function (e) { setCfg(function (p) { var tr = p.taskRouting || {}; var th = tr.complexityThresholds || {}; return Object.assign({}, p, { taskRouting: Object.assign({}, tr, { complexityThresholds: Object.assign({}, th, { low: Math.max(0, Number(e.target.value) || 0) }) }) }) }) } })),
            React.createElement('label', { title: '高复杂度阈值（token）' }, t('task.thresholdHigh'),
              React.createElement('input', { className: 'dsh-mr-num', type: 'number', min: 0, step: 4096, name: 'taskThHigh', autocomplete: 'off', inputMode: 'numeric',
                value: (cfg.taskRouting.complexityThresholds && cfg.taskRouting.complexityThresholds.high) || 65536,
                onChange: function (e) { setCfg(function (p) { var tr = p.taskRouting || {}; var th = tr.complexityThresholds || {}; return Object.assign({}, p, { taskRouting: Object.assign({}, tr, { complexityThresholds: Object.assign({}, th, { high: Math.max(0, Number(e.target.value) || 0) }) }) }) }) } })),
            React.createElement('label', { title: '能力差罚权重（对照任务复杂度）' }, t('task.wCapability'),
              React.createElement('input', { className: 'dsh-mr-num', type: 'number', min: 0, step: 0.5, name: 'taskWCap', autocomplete: 'off', inputMode: 'decimal',
                value: (cfg.taskRouting.weights && cfg.taskRouting.weights.capability) ?? 1,
                onChange: function (e) { setCfg(function (p) { var tr = p.taskRouting || {}; var w = tr.weights || {}; return Object.assign({}, p, { taskRouting: Object.assign({}, tr, { weights: Object.assign({}, w, { capability: Math.max(0, Number(e.target.value) || 0) }) }) }) }) } })),
            React.createElement('label', { title: '延迟权重（紧急任务：高延迟罚、低延迟加分）' }, t('task.wLatency'),
              React.createElement('input', { className: 'dsh-mr-num', type: 'number', min: 0, step: 0.5, name: 'taskWLat', autocomplete: 'off', inputMode: 'decimal',
                value: (cfg.taskRouting.weights && cfg.taskRouting.weights.latency) ?? 1,
                onChange: function (e) { setCfg(function (p) { var tr = p.taskRouting || {}; var w = tr.weights || {}; return Object.assign({}, p, { taskRouting: Object.assign({}, tr, { weights: Object.assign({}, w, { latency: Math.max(0, Number(e.target.value) || 0) }) }) }) }) } })),
            React.createElement('label', { title: '速度权重（紧急/重要任务：低速度罚、高速度加分）' }, t('task.wSpeed'),
              React.createElement('input', { className: 'dsh-mr-num', type: 'number', min: 0, step: 0.5, name: 'taskWSpeed', autocomplete: 'off', inputMode: 'decimal',
                value: (cfg.taskRouting.weights && cfg.taskRouting.weights.speed) ?? 0.5,
                onChange: function (e) { setCfg(function (p) { var tr = p.taskRouting || {}; var w = tr.weights || {}; return Object.assign({}, p, { taskRouting: Object.assign({}, tr, { weights: Object.assign({}, w, { speed: Math.max(0, Number(e.target.value) || 0) }) }) }) }) } })),
            React.createElement('label', { title: '价格权重（同链内相对价差，便宜优先）' }, t('task.wPrice'),
              React.createElement('input', { className: 'dsh-mr-num', type: 'number', min: 0, step: 0.5, name: 'taskWPrice', autocomplete: 'off', inputMode: 'decimal',
                value: (cfg.taskRouting.weights && cfg.taskRouting.weights.price) ?? 0.5,
                onChange: function (e) { setCfg(function (p) { var tr = p.taskRouting || {}; var w = tr.weights || {}; return Object.assign({}, p, { taskRouting: Object.assign({}, tr, { weights: Object.assign({}, w, { price: Math.max(0, Number(e.target.value) || 0) }) }) }) }) } })),
            React.createElement('label', { title: '重要性权重（重要任务：能力不足再罚）' }, t('task.wImportance'),
              React.createElement('input', { className: 'dsh-mr-num', type: 'number', min: 0, step: 0.5, name: 'taskWImp', autocomplete: 'off', inputMode: 'decimal',
                value: (cfg.taskRouting.weights && cfg.taskRouting.weights.importance) ?? 1,
                onChange: function (e) { setCfg(function (p) { var tr = p.taskRouting || {}; var w = tr.weights || {}; return Object.assign({}, p, { taskRouting: Object.assign({}, tr, { weights: Object.assign({}, w, { importance: Math.max(0, Number(e.target.value) || 0) }) }) }) }) } })),
          ) : null,
          React.createElement('div', { className: 'dsh-mr-actions' },
            React.createElement('span', { className: 'dsh-mr-savestatus', role: 'status', 'aria-live': 'polite' },
              saving ? t('common.saving') : (lastSavedAt ? t('save.savedAt', { time: fmtTime(lastSavedAt) }) : t('save.auto'))))),

        // ---- 冷却中候选 ----
        React.createElement('div', { className: 'dsh-mr-card' },
          React.createElement('div', { className: 'dsh-mr-card-head' },
            React.createElement('b', null, t('cd.title')),
        ...
        React.createElement('div', { className: 'dsh-mr-card' },
          React.createElement('div', { className: 'dsh-mr-card-head' },
            React.createElement('b', null, t('caps.title')),
            React.createElement('button', { type: 'button', className: 'dsh-mr-mini', 'aria-expanded': capsOpen,
              onClick: function () { setCapsOpen(!capsOpen) } }, capsOpen ? t('common.collapse') : t('common.expand')),
            React.createElement('span', { className: 'dsh-mr-hint', title: capsWritable ? t('caps.writableTip') : t('caps.readonlyTip') },
              capsWritable ? '' : t('caps.readonly'))),
          capsOpen ? renderCapsBody() : null),

        // ---- 路由编辑 ----
        React.createElement('div', { className: 'dsh-mr-card' },
          React.createElement('div', { className: 'dsh-mr-card-head' }, React.createElement('b', null, t('routes.title')),
            React.createElement('span', { className: 'dsh-mr-hint' },
              t('routes.summary', {
                n: Object.keys(routes).length,
                req: Object.values(statsMap).reduce(function (a, s) { return a + s.requests }, 0),
                sw: Object.values(statsMap).reduce(function (a, s) { return a + s.failovers }, 0),
              })),
            React.createElement('button', {
              type: 'button', className: 'dsh-mr-mini dsh-mr-mini-toggle',
              'aria-expanded': groupOpen,
              onClick: function () { setGroupOpen(!groupOpen) },
              title: t('routes.batchTitle'),
            }, t('routes.batchToggle'),
              React.createElement(groupOpen ? primitives.IconChevronUpOutline14 : primitives.IconChevronDownOutline14, { className: 'dsh-mr-mini-chevron' }))),
          ...
          groupOpen && modelGroups().length > 0
            ? React.createElement('div', { className: 'dsh-mr-groupadd' },
                React.createElement('div', { className: 'dsh-mr-groupadd-head' },
                  React.createElement('span', { className: 'dsh-mr-groupadd-tip' }, t('routes.batchTip')),
                  React.createElement('button', {
                    type: 'button', className: 'dsh-mr-add',
                    onClick: addAllGroups,
                  }, t('routes.batchAddAll'))),
                modelGroups().map(function (g) {
                  var exists = cfg && cfg.routes[g.model]
                  return React.createElement('div', {
                    key: g.model,
                    className: 'dsh-mr-group-item' + (exists ? ' is-exists' : ''),
                  },
                    React.createElement('code', { className: 'dsh-mr-group-model' }, g.model),
                    React.createElement('span', { className: 'dsh-mr-group-provs' },
                      (g.providers.length >= 2 ? t('routes.batchMulti', { n: g.providers.length }) : t('routes.batchSingle')),
                      ' · ',
                      g.providers.join(', ')),
                    exists
                      ? React.createElement('span', { className: 'dsh-mr-group-badge' }, t('routes.batchExists'))
                      : React.createElement('button', {
                          type: 'button', className: 'dsh-mr-add',
                          onClick: function () { addRouteByGroup(g.model, g.providers) },
                        }, t('common.add')))
                }))
            : groupOpen
              ? React.createElement('div', { className: 'dsh-mr-empty' }, t('routes.batchEmpty'))
              : null,
        ...
        React.createElement('div', { className: 'dsh-mr-card' },
          React.createElement('div', { className: 'dsh-mr-card-head' },
            React.createElement('b', null, t('hist.title')),
            historyList.length > 0
              ? React.createElement('button', {
                  type: 'button', className: 'dsh-mr-mini dsh-mr-mini-toggle',
                  onClick: function () { setHistoryOpen(!historyOpen) },
                  title: historyOpen ? t('common.collapse') : t('common.expand'),
                },
    ...
    function apply(ctx) {
      ...
      styleEl.textContent = [
        ...
        '.dsh-mr-sel-task { width: 60px; flex: none; font-size: 11px; padding: 4px 4px; }',
        '.dsh-mr-sel-price { width: 52px; flex: none; font-size: 11px; padding: 4px 4px; }',
        '.dsh-mr-task-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 6px 16px; margin-top: 8px; }',
        '.dsh-mr-task-row { display: flex; align-items: center; gap: 8px; flex-wrap: wrap; }',
        '.dsh-mr-task-row .dsh-mr-check { margin-right: 12px; }',
        ...
        '.dsh-mr-groupadd { display: flex; flex-direction: column; gap: 8px; padding: 10px; border: 1px dashed var(--dsw-alias-border-l2); border-radius: 10px; background: var(--dsw-alias-bg-layer-2); max-height: 360px; overflow-y: auto; overscroll-behavior: contain; }',
        '.dsh-mr-groupadd-head { display: flex; align-items: flex-start; gap: 8px; }',
        '.dsh-mr-groupadd-tip { flex: 1; font-size: 11px; color: var(--dsw-alias-label-secondary); line-height: 1.6; }',
        '.dsh-mr-group-item { display: flex; align-items: center; gap: 8px; padding: 6px 8px; border-radius: 8px; background: var(--dsw-alias-bg-layer-1); border: 1px solid var(--dsw-alias-border-l1); }',
        '.dsh-mr-group-item.is-exists { opacity: .5; }',
        '.dsh-mr-group-model { font-family: ui-monospace, SFMono-Regular, Menlo, Consolas, monospace; font-size: 12px; font-weight: 700; color: var(--dsw-alias-brand-primary); flex: none; max-width: 200px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }',
        '.dsh-mr-group-provs { font-size: 11px; color: var(--dsw-alias-label-secondary); flex: 1; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }',
        '.dsh-mr-group-badge { font-size: 11px; color: var(--dsw-alias-state-success-primary, #34c759); flex: none; padding: 2px 8px; border-radius: 999px; border: 1px solid currentColor; }',
        ...
        scope.slots.inject('conversation.input.left', function () {
          return scope.slots.register({
            ...
            inject: function (sessionId) {
              ...
              // ModelDirectory 无 getSnapshot/subscribe（0.1.2 起在 .store 上）；传 store 供 OverlayStatus 读取
              return { sessionId: sessionId, directory: directory ? directory.store : undefined }
            },
