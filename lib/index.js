...
export function apply(ctx) {
  ...
  const dispose = ctx.on('llm/stream', (options, next) => {
    ...
    const chain = cfg.healthRanking ? rankChainByHealth(picked.chain, health, sessionLoad, cfg.sessionLoadWeight) : picked.chain
  ...
  const disposeAgentRequest = ctx.on('agent/request', async (payload, next) => {
    ...
    try {
      ...
      // 候选未声明时保留会话原有配置。
      if (first.reasoningEffort !== undefined) out.reasoningEffort = first.reasoningEffort
      return out
    } catch (e) {
  ...
  ctx.logger.info('dsh-model-router: 已加载（统一 ModelID 三档路由 + 故障转移 + 健康度择优 + 思考级别 + 会话负载均衡 + 内容感知选档 + 管理面板 API）')
}
