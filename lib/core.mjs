...
// ------------------------------------------------------------------
// 思考级别不支持检测（feature: auto-strip reasoning effort）
//
// 当候选配了 reasoningEffort 但模型/供应商实际不支持时，请求会失败。
// 这类错误的特点：
//   - 宿主 resolveCallConfig 预检拒绝 → code === 'UNSUPPORTED_REASONING_EFFORT'
//   - 供应商 API 返回 4xx + 消息含 reasoning/effort/thinking 关键词
//     （如 OpenAI "reasoning_effort is not supported"、Anthropic
//     "thinking is not available for this model"、各类网关的参数拒绝）
//
// 检测到此类错误后，路由层可安全剥离 reasoningEffort 并在插件内重试
// （不触发宿主层 dsh-llm-retry），同时自动更新配置移除该候选的
// reasoningEffort 字段，避免后续请求重复踩坑。
// ------------------------------------------------------------------
export function isReasoningEffortUnsupported(failure) {
  if (!failure) return false
  // 宿主预检拒绝（resolveCallConfig 抛出的标准码）
  if (typeof failure.code === 'string' && failure.code === 'UNSUPPORTED_REASONING_EFFORT') return true
  const msg = String(failure.message || failure.error || '')
  const status = Number(failure.status)
  const isClientError = status >= 400 && status < 500
  if (!isClientError) return false
  // 供应商 API 4xx + reasoning/effort/thinking 关键词
  if (/reasoning.?effort|thinking|extra_body/i.test(msg)) return true
  // "does not support" / "not available" / "unrecognized" + reasoning/think 关键词
  if (/does not support|not available|unrecognized|unknown.*parameter/i.test(msg) && /reason|think/i.test(msg)) return true
  return false
}
...
export function rankChainByHealth(chain, healthByKey, sessionLoadByKey, sessionLoadWeight = 1) {
  if (!Array.isArray(chain) || chain.length < 2) return chain
  const weight = Number(sessionLoadWeight) || 0
  return chain
    .map((c, i) => {
      const healthScore = candidateHealthScore(healthByKey?.get(cooldownKey(c)))
      // 会话负载维度（least-connections）：运行中会话数越多 → 分数越低 → 排越后。
      // sessionLoadWeight 控制会话数 vs 健康度的权衡；设 0 则退化为纯健康度（向后兼容）。
      const runningCount = sessionLoadByKey?.get(cooldownKey(c)) ?? 0
      const loadPenalty = runningCount * weight
      return { c, i, score: healthScore - loadPenalty }
    })
...
// ------------------------------------------------------------------
// 内容感知选档（feature: content-aware tier selection）
//
// 问题：用户未手动选档（auto）时，默认走 tier2。但 tier2 的候选可能不支持
// 图片输入（图片被静默丢弃或报错），或上下文窗口装不下长文本（每次先打小
// 窗口候选失败再 failover，浪费请求+延迟，且溢出错误污染冷却/健康档案）。
//
// 做法：当无手动档时，根据请求内容自动选档：
//   - 图片：跨所有档找支持图片输入的候选，选运行中会话数最少的候选所在档
//   - 长文本：跨所有档找上下文窗口足够的候选，选运行中会话数最少的候选所在档
//   - 无合适候选 → 返回 null（保持默认档 tier2）
// ------------------------------------------------------------------

// 递归检测 content blocks 中是否含图片块（与 dsh-llm contentHasImage 同源逻辑）
function blocksHasImage(blocks) {
  if (!Array.isArray(blocks)) return false
  for (const block of blocks) {
    if (!block || typeof block !== 'object') continue
    if (block.type === 'image') return true
    if (block.type === 'tool-result' && Array.isArray(block.content) && blocksHasImage(block.content)) return true
  }
  return false
}

// 检测请求 messages 是否含图片块（与 estimateRequestTokens 同源的消息遍历）
export function hasImageContent(options) {
  const messages = options?.messages
  ...
  for (const msg of messages) {
    ...
    const blocks = Array.isArray(content)
      ? content
      : (typeof content === 'string' ? [{ type: 'text', text: content }] : [])
    if (blocksHasImage(blocks)) return true
  }
  return false
}

// 跨档推荐：根据内容需求（图片/上下文窗口）在所有档的候选中找到运行中会话数
// 最少的候选，返回其所在档（tier1/tier2/tier3）。无合适候选返回 null。
//
// 参数：
//   route            —— normalizeRoute 后的路由（含 tier1/tier2/tier3 候选链）
//   requireImage     —— 是否要求图片输入支持
//   needTokens       —— 需要的上下文 token 数（含 reserve）；0 = 不做窗口检查
//   windowOf(c)      —— 返回候选的上下文窗口（number|null；null = 未知 → 不过滤）
//   imageSupportOf(c) —— 返回候选是否支持图片输入（boolean）
//   sessionLoadByKey —— Map<`provider/model`, count>，各候选运行中会话计数
//   opts.margin      —— 窗口可用比例（need > win × margin 即跳过）
//   opts.reserveTokens —— 输出预留 token（叠加在 needTokens 之上）
export function recommendTierByContent(route, requireImage, needTokens, windowOf, imageSupportOf, sessionLoadByKey, opts = {}) {
  const margin = typeof opts.margin === 'number' && opts.margin > 0 && opts.margin <= 1 ? opts.margin : 1
  const reserve = typeof opts.reserveTokens === 'number' && opts.reserveTokens > 0 ? opts.reserveTokens : 0
  const need = needTokens + reserve
  ...
  for (const slot of TIER_SLOTS) {
    ...
    for (const candidate of chain) {
      // 图片要求：候选必须支持图片输入
      if (requireImage && !imageSupportOf(candidate)) continue
      // 上下文窗口要求：need > 0 时检查窗口是否足够
      if (need > 0) {
        const win = windowOf(candidate)
        if (typeof win === 'number' && win > 0 && need > win * margin) continue
      }
      // 候选匹配 —— 取运行中会话数最少的
      const load = sessionLoadByKey?.get(cooldownKey(candidate)) ?? 0
      if (load < bestLoad) {
        bestLoad = load
        bestTier = slot
      }
...
//
// 任务画像（TaskProfile）：importance / urgency / idempotent / complexity /
// tokenBudget。模型属性（候选扩展字段）：location / trust / speed / latency /
// capability / price。本组函数全部为纯函数、确定性、可单测，不含 dsh 依赖。
//
// 分层来源：会话级 override > route.taskDefaults > taskRouting.defaults；
// complexity 可被 autoComplexity（内容 token 估算）覆盖。
// ------------------------------------------------------------------

export const CAPABILITY_LEVELS = { low: 0, medium: 1, high: 2 }

// 候选能力档位 → 数值。unknown/缺省 → 1（medium，中性：不参与惩罚）。
export function capabilityLevel(candidate) {
  const level = candidate && CAPABILITY_LEVELS[candidate.capability]
  return typeof level === 'number' ? level : 1
}

// 任务复杂度 → 数值（与 capabilityLevel 同一标尺，便于直接比差）。
export function taskComplexityLevel(profile) {
  const level = profile && CAPABILITY_LEVELS[profile.complexity]
  return typeof level === 'number' ? level : 1
}

// 自动复杂度：按请求体量（token 估算）推断。阈值可配，默认由调用方传入。
// 无消息/0 token → 'low'；超过 high 阈值 → 'high'；超过 low 阈值 → 'medium'。
export function autoComplexityFromTokens(tokens, thresholds = {}) {
  const n = Number(tokens) || 0
  const high = Number(thresholds.high)
  const low = Number(thresholds.low)
  if (Number.isFinite(high) && high > 0 && n >= high) return 'high'
  if (Number.isFinite(low) && low > 0 && n > low) return 'medium'
  return 'low'
}

export const TASK_PROFILE_KEYS = ['importance', 'urgency', 'idempotent', 'complexity', 'tokenBudget']

// 分层合并任务画像：session override > route.taskDefaults > 全局 defaults，
// 逐键取首个非空值，最后兜底默认。autoComplexity 开启且 complexity 未显式
// 指定时，用内容 token 估算覆盖。返回完整画像（五个键齐全）。
export function resolveTaskProfile({ session, route, global, autoComplexity = false, neededTokens = 0, thresholds } = {}) {
  const out = {}
  for (const key of TASK_PROFILE_KEYS) {
    const value = (session && session[key]) ?? (route && route[key]) ?? (global && global[key])
    if (value !== undefined && value !== null && value !== '') out[key] = value
  }
  out.importance = out.importance ?? 'normal'
  out.urgency = out.urgency ?? 'not-urgent'
  out.idempotent = out.idempotent ?? true
  out.complexity = out.complexity ?? 'unknown'
  out.tokenBudget = Number.isFinite(Number(out.tokenBudget)) ? Number(out.tokenBudget) : 0
  if (autoComplexity && (out.complexity === 'unknown' || out.complexity === undefined || out.complexity === '')) {
    out.complexity = autoComplexityFromTokens(neededTokens, thresholds)
  }
  return out
}

// token 预算硬过滤：budget > 0 且候选 price > 0 时，估算成本
// price × neededTokens / 1e6（每百万 token 计价）；超预算跳过。
// budget <= 0 = 不限；price <= 0/未知 = 不参与过滤（放行尝试）。
// 返回 { chain, skipped }；全过滤的回退决策由调用方做（同 filterChainByContext）。
export function filterChainByBudget(chain, neededTokens, priceOf, budget) {
  const budgetNum = Number(budget)
  const need = Number(neededTokens)
  if (!(budgetNum > 0) || !(need > 0)) return { chain: Array.isArray(chain) ? chain.slice() : [], skipped: [] }
  const kept = []
  const skipped = []
  const priceOfC = priceOf || ((c) => (c && typeof c.price === 'number' ? c.price : 0))
  for (const candidate of chain || []) {
    const price = Number(priceOfC(candidate)) || 0
    if (price > 0) {
      const cost = (price * need) / 1_000_000
      if (cost > budgetNum) {
        skipped.push({ candidate, price, cost, budget: budgetNum })
        continue
      }
    }
    kept.push(candidate)
...
export const TASK_WEIGHT_DEFAULTS = { capability: 1, latency: 1, speed: 0.5, price: 0.5, importance: 1 }

// 任务匹配排序：在健康度排序之后链式调用，按任务画像重排候选链。
// 评分规则（确定性、可单测）：
//   - 能力差罚：|候选 capability - 任务复杂度| × wCapability
//   - 重要任务：能力低于复杂度需求再罚（complexity-cap）× wImportance
//   - 紧急：latency=high 重罚、medium 罚、low 加分（× wLatency）；unknown 不罚
//   - 紧急/重要：speed=low 罚、high 加分（× wSpeed）
//   - 价格：同链内相对价差归一化后 × wPrice（便宜优先）
// 稳定排序：同分保持原顺序（用户显式排序仍然优先）。
export function rankChainForTask(chain, profile, weights = {}, priceOf) {
  if (!Array.isArray(chain) || chain.length < 2 || !profile) return chain
  const w = { ...TASK_WEIGHT_DEFAULTS, ...weights }
  const complexity = taskComplexityLevel(profile)
  const urgent = profile.urgency === 'urgent'
  const important = profile.importance === 'important'
  const priceOfC = priceOf || ((c) => (c && typeof c.price === 'number' ? c.price : 0))
  const prices = chain.map((c) => Number(priceOfC(c)) || 0)
  const priceMin = prices.length > 0 ? Math.min(...prices.filter((p) => p > 0)) : Infinity
  const priceMax = Math.max(...prices)
  return chain
    .map((c, i) => {
      let score = 0
      const cap = capabilityLevel(c)
      const capPenalty = Math.abs(cap - complexity) * (Number(w.capability) || 0)
      score -= capPenalty
      if (important && cap < complexity) score -= (complexity - cap) * (Number(w.importance) || 0)
      if (urgent) {
        const lat = c.latency
        const wLat = Number(w.latency) || 0
        if (lat === 'high') score -= 2 * wLat
        else if (lat === 'medium') score -= wLat
        else if (lat === 'low') score += 0.5 * wLat
      }
      if (urgent || important) {
        const sp = c.speed
        const wSp = Number(w.speed) || 0
        if (sp === 'low') score -= 1.5 * wSp
        else if (sp === 'high') score += 0.5 * wSp
      }
      const wPrice = Number(w.price) || 0
      if (wPrice > 0 && priceMax > priceMin) {
        const price = prices[i]
        if (price > 0) score -= ((price - priceMin) / (priceMax - priceMin)) * wPrice
      }
      return { c, i, score }
    })
    .sort((a, b) => (b.score - a.score) || (a.i - b.i))
...
// 幂等控制重试：idempotent=false（非幂等任务）→ 任何失败都不重试（防止
// 重试造成重复副作用）。idempotent 缺省/true → 委托调用方既有判定。
// wouldRetry 由调用方按失败类型算出（如 isTransientFailure + 重试次数上限）。
export function shouldRetryForTask(profile, wouldRetry) {
  if (profile && profile.idempotent === false) return false
